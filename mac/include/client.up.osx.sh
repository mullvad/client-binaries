...
sed -e 's/^[[:space:]]*[[:digit:]]* : //g' | tr '\n' ' '
	...
	if ${ARG_MONITOR_NETWORK_CONFIGURATION} ; then
        ...
        if [ "${LEASEWATCHER_TEMPLATE_PATH}" != "" ] ; then
            # MULLVAD
            # Rather than using a working .plist file and replacing the path,
            # we use our custom .plist.template file which simple has the
            # search path replaced with DIR.
            sed -e "s|DIR|${TB_RESOURCES_PATH}|g" "${LEASEWATCHER_TEMPLATE_PATH}" > "${LEASEWATCHER_PLIST_PATH}"
		fi
        # MULLVAD
        # A small addition to the script output which lets us see if we are
        # invoking the right file.
        logMessage "MULLVAD: Running 'launchctl load $LEASEWATCHER_PLIST_PATH'"
...
# MULLVAD
# Use the current working directory to find the other application resources
# (the only relevant one being LeaseWatch.plist.template) rather than an
# absolute path to the installed application. This is mostly to make it easier
# while developing.
# readonly TB_RESOURCES_PATH="${ARG_TB_PATH}/Contents/Resources"
readonly TB_RESOURCES_PATH=$(pwd)
...
if [ "${ARG_TB_PATH}" = "/Applications/Tunnelblick.app" ] ; then
    # MULLVAD
    # Slightly modified behavior from the original. First of all this branch
    # is always executed since ARG_TB_PATH is hardcoded. The template file is
    # an actual template rather than a working file, with the string DIR where
    # the path should be. The resulting .plist will be removed when the
    # connection is torn down.
    readonly LEASEWATCHER_PLIST_PATH="${TB_RESOURCES_PATH}/${LEASEWATCHER_PLIST}"
    readonly LEASEWATCHER_TEMPLATE_PATH="${TB_RESOURCES_PATH}/${LEASEWATCHER_PLIST}.template"
